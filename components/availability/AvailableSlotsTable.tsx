                ...
                <table className="min-w-full table-auto border-collapse bg-gray-500 border border-gray-300 rounded shadow">
                    <thead className="bg-gray-800 text-white">
                        <tr>
                            ...
                            <th className="text-left px-3 py-2 text-sm sm:text-base border-b">Date</th>
                    ...
                    <tbody>
                        {currentRows?.map((slot) => (
                            <tr key={slot.id} className="hover:bg-gray-100">
                                ...
                                <td className="px-3 py-2 border border-gray-300">{new Date(slot.start_date).toDateString()}</td>
                                <td className="px-3 py-2 border border-gray-300">{new Date(slot.start_date).toLocaleTimeString()}</td>
                                <td className="px-3 py-2 border border-gray-300">{new Date(slot.end_date).toLocaleTimeString()}</td>
            ...
            <div className="flex flex-wrap justify-center mt-6 gap-2">
                ...
                {Array.from({ length: totalPages }, (_, i) => (
                    <button
                        ...
                        className={`px-3 py-1 rounded ${currentPage === i + 1 ? 'bg-blue-600 text-white' : 'bg-gray-500 text-white hover:bg-gray-700'
                            }`}
                    >
                        {i + 1}
                    </button>
